...
| Date | Change | Reason |
|---|---|---|
| 2026-08-24 | Phase 2 assessed against the code before building; most of it is superseded, and the surviving half is built | Plan §0.1 says to verify the plan against the repo and say so BEFORE starting. Three of §4's five deliverables are dead on answers Addie had already given: there is no Twilio confirmation (Q-005 — it is the RSVP email), there is no `disputed` state (Q-006 — `portalRsvp` accepts three answers and R-005 was retired for exactly this), and per-option crew check-off needs the crew portal, which is out of use this season. What survived is the half nothing had noticed was missing: the send recorded NOTHING, so nobody could say who had been told, what they were told, or whether it was still true. `confirmationSentAt` / `confirmationShown` are written by the send and read by a chip on the customer record (R-012) and a filter in All Customers. §4.3's replacement is Q-015 and is not guessed at. |
| 2026-08-24 | Phase 1 landed: R-001, R-002 and R-004 now have real enforcement, and R-003's `read` half is narrower | The registry stopped being a spec nothing read. Five artifacts generate from `js/options.js` — both quote forms, the RSVP confirmation, the crew sheet, both build sheets and the invoice — so R-001's "and nowhere else" is now a fact about the code rather than an instruction. R-002 is enforced in one place (`display()`) instead of three spellings of nothing on one sheet (`?`, an em dash, a blank cell), and a whitespace-only value was found slipping past it. R-004 is unchanged and still audited. R-003 stays `read` because the audit checks COHERENCE, not arrival: `consumers` is the declaration, so deleting one is coherent and wrong — the frozen AGREED map in `options-audit.test.js` is what catches that, and it is a test, not the rule. No rule TEXT changed. |
| 2026-08-24 | Noted against R-016 in passing (no text change) | "Generation beats verification" produced two bugs this session that no test had found and no test could have: a customer's answer to the eaves question was collected by the form and dropped by `quoteSaveDetails`, and `outletTimer` was written as a definite "No" by the server for anyone arriving through an emailed link while the browser deliberately left it blank. Both surfaced the moment the form was generated from the registry rather than hand-written — the registry declared a field the server had never been told about. |
