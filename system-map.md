...
- **Filter, then select everyone under the filter** (Automation Emails → Preview & Send, rebuilt 2026-09-04). Dax: *"in automated emails make it so you can filter who you see then you can select all so it selects everyone under the filter you chose."* Both halves of that sentence already existed and the send still reached the wrong people, for two separate reasons.
  ...
  - ⚠ **AND THE TWO "LAST YEAR" FILTERS COULD NOT SEE LAST YEAR AT ALL** (fixed 2026-09-05). Dax: *"when i push paid last year or didnt pay last year is shows no members match these filters which is not true."* He was right, and the fault was not in the filter — it was one panel away. `audiencePaidLastYear` and `audienceOrderedLastYear` read `yearlySnapshotsCache`, which was filled by `loadFinancialTrackers` inside the **money** group, and the money group is pulled by the **Dashboard alone**. So: open admin, go straight to Automation Emails, pick *Paid last year*, and the cache is empty, the snapshot answers null for everybody, and both filters match nobody — with a real snapshot sitting unread in Firestore. Open the Dashboard first in the same session and the identical click works, which is what made it look random.
    - ⭐ **The snapshot is its own loader and its own group now** (`loadYearlySnapshots`, group `snapshots`), asked for by `automation:` as well as `dashboard:`. ⚠ It is listed BESIDE `money`, never inside it: `ensurePanelData` de-dupes by GROUP NAME, not by loader, so a function named in two groups opens two `onSnapshot` listeners on one collection — the listener-accumulation shape that once cost 2815 writes on a single drag.
    - ⚠ **The comment above `lastSeasonSnapshotFrom` predicted this exact fault and named the wrong panel.** It says the arrears backfill reads the collection fresh "because `yearlySnapshotsCache` is only filled by the money group, which the Invoices panel does not load. Reading the cache there would report 'no snapshot' when one exists, which is a confident wrong answer." The same trap was live one panel over, on a filter shipped after that warning was written. Check a cache's READ SITES, not just its panel.
    - ⭐ **And the empty list now says which of the three it is.** It read *"No members match these filters"* — the one sentence guaranteed to send somebody hunting for a broken filter. The count line carried the real explanation and the list contradicted it, and the list is what gets read. It names the filter that emptied the screen, says the snapshot is what is missing, and says how to get the list back. ⚠ It must NOT fire whenever a filter legitimately matches nobody — that is checked both ways.
    - ⚠ **The auto-save rode on the same listener**, so on a machine where nobody ever opened the Dashboard, last season was never snapshotted either: the filters had nothing to read AND nothing was writing it. Fixed by the same move.
    - Suite 307 §8–9 of `run-all.js`; 6 sabotages red-checked. Suite 21 already proved every `PANEL_DATA` key is a real panel and every group it names has a handler — which is exactly why it stayed green through this bug, and why "Automation Emails asks for the snapshot AT ALL" is asserted separately.
