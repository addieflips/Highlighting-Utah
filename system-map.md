...
- ⭐ **A member who changes something about their house gets an auto-reply saying we have got it** (2026-09-16, [[EM-18]]). Addie: *"can we get an automation email set up for someone who makes a change in the member portal. Like saying something like we'll make sure to make this change on your house."* It fires on three of `portalSave`'s five sections — **lights**, **preferences** and **sides** — and names what actually changed: the colours, the wire colour, the timer, which outlet, when they want it hung, their note, or which sides. She turns it on and picks the wording under **Automation Emails → Templates → Auto-reply when a member changes something**; it is **shipped switched off**, because it mails real customers the moment it works.
  ...
  - ⭐ **AND THEN THEY ARE ASKED AGAIN — Any, Green or White, with instructions** (2026-09-18, [[OPT-21]], superseding [[OPT-12]]). Addie: *“They should see Any, Green, White. With instructions on what to pick. If they click any or keep it at any then it should allow us to pick and require us to pick the wire.”* The question is back on the **detail form** — the one a new customer fills in after approving — as three pills in her order, with **Any** already picked and a line telling her what to choose: white wire where the gutters and trim are white or light, green where they are dark, and Any if she would rather we decided. ⛔ **“Any” stores nothing at all, and that one line is the whole design.** It is not a colour — it is her saying she has no preference, and the way to record that is to record nothing. A blank is already what the rest of the app understands: the warehouse heading reads **Check lights** ([[WH-35]]), the **Pick a Wire Colour** System Message with a picture of the house is raised ([[WH-40]]), and the **Convert to Customer** popup will not convert until the office picks one ([[WH-43]]) — which is the second half of her own sentence, already built the same day. ⚠ **This reverses [[OPT-12]] and its argument is what makes it safe.** That ruling was refusing an **invented** answer: the old control defaulted to `Any` and **stored the word**, so most quotes carried a cord nobody had chosen. The question was never the fault. ⛔ **Absent, not blank.** Neither the browser nor the server writes `wireColor: ''` — a posted blank would **erase** a colour a re-quote had prefilled off the member's own record. ⚠ **Two copies of one rule, compared**: `qdWireChoice` in index.html decides what the radio posts, `quoteWireChoiceServer` decides what is stored (that callable is reached with nothing but a quote token, so the browser's answer is a suggestion), and `wire-pick.test.js` runs both side by side. ⛔ **The Member Portal is untouched** — still no wire control on the Changes tab, still out of `PORTAL_WRITE_FIELDS` and `PORTAL_READ_FIELDS`. She asked about the form a NEW customer fills in; offering *Any* to a member who already has Green would be offering to erase it. ⚠ **And it puts back a property the White sweep already leans on**: `wireSweepClassify` keeps a White that came from a quote *because* that form defaults to Any, so a White on one is a customer ticking White. That sentence stopped being true when the control was removed; it is true again. `test/quote-wire-choice.spec.js` drives it in a real browser, because every claim here is a control on screen or a field in a save.
