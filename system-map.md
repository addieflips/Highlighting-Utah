...
- **`sendNightlyInvoices`** — cron, 7 PM Mountain daily (`0 19 * * *`). No-ops unless the automation toggle in `settings/nightlyInvoiceAutomation` is on. Bills any completed-but-uninvoiced house, logs to `nightlyInvoiceLog`, and tries to text the owner a summary via Twilio — which does not arrive, see below.
...
- ⚠ **Nightly-run failures/results are supposed to text the owner, and do not.** `twilioSendRaw` posts to Twilio on an account Highlighting Utah has never had; it returns `{ok:false}` and never throws, so the alert has always failed silently and the run carries on regardless. It was written as a separate channel that would survive email breaking, and it is not one — **if email breaks, nothing tells you.** The summary is still written to `nightlyInvoiceLog` either way. We text from Google Voice, which has no send API, so a server cannot send this at all; reaching a phone would mean an account somewhere that can.
...
- **"Send as text instead" on a quote does not send anything.** It copies the message, opens that customer's Google Voice conversation, and waits — the quote is marked sent only when the office presses *I sent it*. It cannot be otherwise: we text from Google Voice and Google Voice has no send API, so no button in a browser can put a text on the wire.
  ...
  - The `sendSms` function is still deployed and still Twilio-only. Nothing calls it. Do not wire a button back to it.
  - ⚠ **The owner-alert texts are the same story and are still broken.** `twilioSendRaw` sends the nightly invoicing summary to `alertPhone`, returns `{ok:false}` on failure and never throws — so those texts have silently never arrived. The run itself is unaffected and the same summary is in `nightlyInvoiceLog`.
