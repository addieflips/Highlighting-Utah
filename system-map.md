...
11. **RSVP "no"** — records the answer and takes them off any upcoming route. **It does NOT touch their lights** (changed 2026-09-03, RS-51). Dax: *"when they click no they move to maybe next year and only go to archive if they actually go through the process of canceling their lights in member portal."* `seasonBadgeKey` already reads a bare `no` as **Maybe Next Year**, so that is what the office sees.
   ...
   ⭐ **PAYING THIS SEASON IS A YES** (2026-09-24, [[RS-77]]). Addie: *"if they paid this season it should approve there quote"*. Every time an invoice changes, `approvePaidThisSeason` finds anybody who has paid money toward this season (deposit beyond any carried debt) and has not answered, and records a dated **Yes** for them, noted in the activity log. They then read Confirmed, drop off the pending lists and RSVP sends, and go on the schedule. A payment that only clears **last season's** debt is not a yes and still raises the *Paid Last Season — Still Needs to RSVP* note. An answer already given — Yes, No, Back Next Year — is never overruled. This replaces [[RS-40]]'s "money is not consent" for this season's money. Nobody already emailed the RSVP is sent it again ([[EM-04]]). ⚠ **And Unanswered is pending, not answered** — it had been read as an answer in five places, which dropped those customers from the RSVP plan, the text list, the link sheet and the pending phone numbers; `rsvpHasAnswered` fixes all five, and the server's gate matches.

   ⭐ **BACK IN WHICH YEAR** (2026-09-24, [[RS-76]]). Edit Customer shows a **Back in which year?** box whenever the RSVP dropdown says Back Next Year, for missionaries and anybody away more than one season. It is stored as `backInYear`, and the badge on All Customers reads **Back in 2028** instead of Back Next Year. Nothing else reads it until that year: they stay out of the season, off the bill and unasked, exactly as Back Next Year always has. When that year comes, **Move everyone to Unanswered** clears their Back Next Year badge along with the answer, so they are pending like everybody else and get the RSVP; its confirm names them first. ⭐ **The default is the next year** (2026-09-25, [[RS-79]]): with no year picked, they are back the year after they said Back Next Year (from `maybeNextYearAt`, else `rsvpRespondedAt`), and the box opens on next year. Only a record with no date at all has no year.
...
- ⭐ **2026-09-18, the quote flow asks WHICH too** ([[OPT-23]]). Addie: *"on quotes/ Requotes can we make front, left, right, back multiple optional choose"*, and *"is there a place on the quotes form that allows them to choose front, left, right, back with instructions that this is from the street view?"* The answer to the second was **no** — the free quote form has not asked about sides since 2026-09-03 (deliberately; it is in front of a stranger deciding whether to ask for a price, and that has not changed), and the **Install Details** form they fill in after approving asked only *how many*, as four radios. The Member Portal's Sides tab was the only place asking by name, so a customer who never opened the portal was never asked and the crew sheet printed a number.
  - ⛔ **The count is derived from the ticks, never asked separately.** `houseSides` still drives the price and the re-quote flag exactly as before; what changed is that the two can no longer disagree, because there is one read of the ticks and the count is its length. Asking both questions is how a record reaches the state `printSidesCell` refuses — a list of two under a count of three.
  - ⭐ **The street sentence is the portal's, word for word** ([[OPT-03]]). The two viewpoints are mirror images, so a named side with no viewpoint lights the wrong half of a roof on a coin toss — and one question asked two ways is how the two start meaning different things.
  - ⛔ **The names had to join the server whitelist.** `quoteSaveDetails` keeps one, and the emailed-link path that goes through it is the common one — the browser would have sent the names and the update would have dropped them, with nothing on any screen saying so. The same warning was already written above `houseSides` there.
  - ⭐ **One sanitizer** (`sanitizeSideNames`), lifted out of `portalSave`. A red-check then found a **third** copy already living in the re-quote comparison, which decides whether somebody is re-quoted by comparing two lists as *sequences* — its own ordering would have made a real swap of Left for Right read as no change, silently.
  - ⚠ **At least one side is refused, not defaulted**, and **Front is pre-ticked** — her *"1 side should be default"*, said by name. The office forms are untouched: Add and Edit Customer still ask a count and a list separately, with the fits-the-count guard between them.
...
- ⭐ **AND A PHOTOGRAPH IS SHRUNK BEFORE IT EVER GETS THERE** (2026-09-18, [[PROC-35]]).
...
- ⭐ **A member who changes something about their house gets an auto-reply saying we have got it** (2026-09-16, [[EM-18]]). Addie: *"can we get an automation email set up for someone who makes a change in the member portal. Like saying something like we'll make sure to make this change on your house."* It fires on three of `portalSave`'s five sections — **lights**, **preferences** and **sides** — and names what actually changed: the colours, the wire colour, the timer, which outlet, when they want it hung, their note, or which sides. She turns it on and picks the wording under **Automation Emails → Templates → Auto-reply when a member changes something**; it is **shipped switched off**, because it mails real customers the moment it works.
  ...
  - ⛔ **My Info and cancellations send nothing, on purpose.** `info` is phone, email and gate code — not a change to the house, and [[MSG-18]] already settled that tab is deliberately quiet (*"changing gate code or phone number should not notify us"*). `cancel` is somebody leaving, and thanking them for a change we are going to make is the wrong thing to send to the one person who just said no.
  - ⛔ **It asks the SAME question the office history asks.** `describePortalChanges` was split so both it and the email call `portalChangedFieldNames` — two copies is how the activity log records a change the confirmation never mentions. And a hand-rolled comparison is not merely duplication here, it is **wrong**: the preferences form posts all six of its fields on every save whether they were touched or not, and an unticked box arrives as `''` while the record stores `false`, so a naive diff emails the whole book about changes nobody made.
  - ⭐ **A note written for the warehouse, on both of the sheets they work from** (2026-09-17, [[WH-37]]). Addie: *"on costumers we need to be able to add notes to warehouse that updates when we print off warehouse pages."* Add Customer and Edit Customer have a **Notes for the warehouse** box; it is stored as `warehouseNote` on the customer and printed in the Notes column of **both** build sheets. ⛔ **The thing that was actually broken was the paper, not the typing.** Permanent Notes has always printed on the **Warehouse tab's** sheet — and the **Printing tab's** build sheet, which is what *print off warehouse pages* means, **had no Notes column at all**, so anything typed for the warehouse was visible on one screen and invisible on the pages. ⚠ **Its own box rather than a second use of Permanent Notes**, because the crew read that one on their route sheet: a message about which shelf a bin is on would ride out in a van with it. ⚠ **The warehouse note prints first and Permanent Notes still follows it** — that cell has always carried the general note and somebody is relying on it. ⚠ **One function (`whNotesCell`) fills the cell on both sheets**, the same rule as the bins count: two sheets deciding this separately is how they start disagreeing. ⚠ **A top-up still leads with GOES INTO THE BIN THEY ALREADY HAVE**, whatever anybody typed — that line is what stops a second bin being made for a house that already has one. ⛔ **Changing the note never queues a bundle rebuild.** `warehouseNote` is deliberately not one of the `WAREHOUSE_BUILD_FIELDS`; a note is not glass, and correcting a typo must not send somebody to make a second set of lights. ⚠ **This reverses part of Addie's own 2026-08-24 trim of the printed sheet**, which cut it down so it would read on landscape paper — the newer instruction wins (R-024) and Notes goes **last**, where a wide free-text column cannot push a scannable one off the edge.
  - ⭐ **A build added in the warehouse now changes the customer, and stops being its own heading** (2026-09-21, [[WH-44]]). Addie, showing a `Doug Smith — extra request` card beside an ordinary `Red, Warm White — Green wire` group: *"I dont want it to look like the first picture but I want all of them to look like the second picture even if I add them in warehouse. If I add them in warehouse it should also update there lights in member portal, and costumers."* ⛔ **It used to write a second record and tell nobody.** A request was a `warehouseExtras` document and nothing else, so the customer's own record still held the old colours — the Member Portal showed them, Customers showed them, and the new ones lived only on a warehouse row no other screen reads. The form said so out loud, *"Their customer record is not changed either way"*, and that is the sentence she overruled. ⭐ **`whAddRequestToCustomer` writes the colours and the wire onto the house** and queues the build, so the heading fixes itself: the house **is** a house in that colour and wire group, and it gets the bundle line, the REQUEST badge and the **Mark Completed** box that a titled group suppressed. That suppression is what made the old heading costly — a real job sat on the tab with no way to tick it off. ⚠ **Buffer stock is untouched**: with no house picked there is nobody to update, so that path still writes a `warehouseExtras` row exactly as before. ⚠ **A person with two requests now reads under two headings**, which the old grouping deliberately avoided; that reasoning is kept in the code, struck rather than deleted, because it is the cost she accepted rather than a mistake. ⭐ **It charges the $30.** Asked directly, Addie: *"Yes should charge the fee."* It goes through `applyLightChange` and the same three-answer popup as Edit Customer and the All Customers row panel, and **Cancel writes nothing at all** — what a customer pays must not depend on which box somebody typed into. ⚠ **It also puts them on the colour-change list.** The first draft left that off, reasoning that a request typed in the warehouse is already on the warehouse's build list; the colour-change gate caught it and the older rule won, because a colour change that reaches no list is silence and their existing bin still has to be dealt with. ⭐ **The bundle count you type wins** (`buildBundleOverride`) — *"The number I type wins but the automatic number should still come up."* The footage answer still fills the form in, and the typed one is **cleared when the bundle is marked done**, so it cannot quietly govern next season's build. It is stored only when it disagrees with the derived count. ⭐ **And the 72-hour hold comes for free**: *"Yes then it will give them 72 hours until they get scheduled."* The build stamp goes through `stampBuildQueued`, `scheduleHoldEndsMillis` reads `lightsQueuedAt`, and that hold is longer than the 48-hour route lock the fee rule sets — so nothing new decides when they can be scheduled.
  - ⭐ **The warehouse notes are shown on the warehouse tab, and can be typed there** (2026-09-21, [[WH-45]]). Addie: *"Can we make it so that Darlene price notes come up under her name if I add notes to warehouse and be able to add those notes inside warehouse under there name."* ⛔ **Both halves were missing, and the second is the one that bit.** `warehouseNote` and `warehouseOneTimeNote` have printed on both build sheets since [[WH-38]], but the warehouse **tab** drew only the general Permanent Note — so a note written *for* the warehouse was invisible on the warehouse's own screen until somebody printed, which is indistinguishable from the field not working. And it could only be typed on Add Customer or Edit Customer. ⭐ **Each house row now carries a Note button** that opens two boxes under their name — **Notes for the warehouse** (stays on their record) and **This build only** (clears itself when the bundle is marked done) — and saves straight to the customer. ⚠ **The screen reads `whNoteText`, which is what the printed sheet reads**, so a note typed on the tab is on the next sheet by construction; a second composer here is how the tab and the paper start disagreeing about one house. ⚠ **The one-time box is labelled as such**, because an unlabelled one-time note is indistinguishable from a standing instruction and goes on being obeyed after the build it belonged to was made. ⚠ **The panel is folded away by default** — this list is scanned while building, and two textareas a house would bury the colours and the bin number somebody is looking for. ⚠ **Saving does not redraw the list**: the tab repaints from the `jobAddresses` cache and a repaint would destroy the box being typed in, so the write is mirrored into the cache instead. ⛔ **And a note never queues a rebuild** — `warehouseNote` stays outside `WAREHOUSE_BUILD_FIELDS`, because correcting a typo must not send somebody to make a second set of lights.
  - ⭐ **Searching a name in the warehouse pulls the person up** (2026-09-21, [[WH-46]]). Addie: *"Also when I type in a name in warehouse it should pull them up just not that they are in the queue."* ⛔ **The search was never the broken half**, and that was measured rather than assumed: the box has always read the whole book rather than the build list, and says so when nothing matches. What came back was a single sentence about whether they were queued — the question somebody standing at the shelf has already asked. ⭐ **The row now carries what a build needs**: their customer number, the address, the colour and wire chips, how many bins, whatever is in their warehouse notes, and an **Open** button onto the record. ⚠ **The build status keeps its own line and colour** underneath, because *why is this house not in my queue* is still a real question — it is just no longer the only answer. ⚠ **The chips and notes come from `whHouseFactsHtml` and `whNoteText`**, the same two the queue rows and both printed sheets read, so this screen cannot start disagreeing with the sheet somebody is holding.
  - ⭐ **Crew notes stay with the crew, warehouse notes with the warehouse** (2026-09-23, [[WH-48]]). Addie: *"want only crew to show for crews and warehouse to show for warehouse"*. The warehouse side used to add the customer's **Permanent Notes** (the crew's box) after the warehouse note, on the tab and on both printed build sheets. `whNoteText` now reads only **Notes for the warehouse** and **This build only**. The crew sheet (`printCrewNotes`) reads only Permanent Notes, One-Time Notes and the outlet note, and never the warehouse boxes. ⚠ This narrows [[WH-37]], which had kept Permanent Notes on the warehouse sheet deliberately.
  - ⭐ **The warehouse note is drawn big enough to be seen** (2026-09-23, [[WH-47]]). Addie: *"I need the warehouse note to be more well seen. It's to small right now"*. It was 12px grey at the bottom of the row, the same as the bundle count. `whNoteBoxHtml` now draws it in a tinted, labelled box at 14px — the one-time note first and in red, the permanent note bold — on the build rows and in the find box alike. ⚠ The words are the same fields in the same order as `whNoteText`, so the screen still says exactly what the printed sheet says; the sheets themselves are unchanged.
  - ⭐ **And there are two of them — permanent, and this build only** (2026-09-17, [[WH-38]]). Addie: *"it needs to have permanent notes or one time notes."* The same pair the customer notes already are. The one-time note prints **first** and is labelled **THIS BUILD ONLY:**, because it is the only line in the cell that is true this morning and false next week — unlabelled it is indistinguishable from a standing instruction and would go on being obeyed after the bundle was made. ⛔ **It is cleared the moment either Mark Done is pressed**, and the permanent one is not. ⚠ **Both Mark Done buttons now write through one function, `whBuiltUpdates`** — one house and a whole colour group used to write that object out separately, which was survivable while it was three fields nobody was adding to, and stopped being survivable the moment a note had to be *cleared* there: cleared on one path and left behind on the other is an instruction that outlives its build.
  - ⭐ **And the warehouse now says “Check lights” where nobody ever chose a wire** (2026-09-16, [[WH-35]]). Addie: *“anyone that says white wire in warehouse right now lets get rid of that completley and just show them as Check lights which means check lights to see what there wire color currently is.”* `whWireLabel` read *whatever is stored, or else White* — so every house with nothing on file was shown as **White wire** and, because the build grouping is built on that label, was **built into the White pile**. That is older than the portal bug above and affected more houses. Unconfirmed houses now read **Check lights** on the chips, both build sheets, the recycle queue and the crew sheet, and form **their own build group** rather than being folded in with the real White ones. ⚠ A house whose wire IS recorded as White still says White — so confirming one is an ordinary edit and that row stops asking. ⛔ It stores nothing: no record is rewritten, which is why it was preferred over un-picking White across the book — it cannot lose a real answer. ⚠ Buffer stock is untouched; a warehouse extra with no wire already takes its own heading.
  - ⭐ **And a house whose COLOURS nobody saved is “Check lights” too — the Waiting on light colours block is gone** (2026-09-18, [[WH-41]]). Addie: *“On warehouse Waiting on light colours should not be a thing. We just add if they want a new wire with the light colors they have saved or just save wire color with check lights if thats not saved or add timer. There should not be a Waiting on light colours section though.”* The block listed every house flagged for a build with nothing in either colour field, under a heading reading *“Nothing can be made up until somebody fills them in”* — and that sentence is what she is objecting to, because it is false about the job she actually does. Such a house now heads an ordinary build pile named by `whCheckLightsKey`: **Check lights — Green wire** where the record knows the wire, and just **Check lights** where it does not.
    - ⛔ **It narrows [[WH-18]] and reverses nothing in [[WH-20]].** Nobody is dropped and nothing is hidden. Those houses keep their badge, their bins, their Notes cell, their **Mark Done**, their **Not needed** and their row on *both* printed build sheets — typed **House** now rather than **Blocked**, on a page of their own.
    - ⚠ **One vocabulary, not two.** The wire half has read *Check lights* since [[WH-35]] two days earlier and means the same thing: go and look at the bundle. Where **both** halves are unknown it is said **once** — `whGroupKey` composes “&lt;colours&gt; — &lt;wire&gt; wire”, so the obvious version heads that pile *“Check lights — Check lights wire”*, which reads as a rendering fault. Two thirds of the book has no wire on file ([[WH-36]]), so that is the commonest of these headings rather than an edge case.
    - ⛔ **No bulbs are ordered from it.** `computeColorDemand` is deliberately still gated on the colours: the build list is *who to make something for*, the demand total is *what glass to buy*, and counting a Check lights house there would have to invent a colour. The two lists disagreeing here is the right answer, not drift.
    - ⚠ **[[WH-27]]'s way out moved rather than being dropped.** *Only needed a timer* lived on the blocked row; it rides the build row now, offered only where there are **no colours AND** a timer is on file — anywhere else it is a control that means nothing.
    - ⚠ **And the tab's “Nothing needs to be built right now” note was widened in the same line.** It asked only about the colour groups and the blocked list, so a book whose only outstanding warehouse job was a timer showed that note with the **Timers** block below it never drawn. It asks about all three lists now.
    - ⚠ **Nothing on any record changed**, exactly as [[WH-35]] did not: this is what the warehouse is *told*, not what is *kept*, so filling a house's colours in is an ordinary edit and its heading heals itself.
  - ⭐ **And the White that was never chosen can be cleared, one sweep, on her evidence** (2026-09-16, [[WH-36]]). Addie, asked whether a blank Wire column just means White: *"Blank means we just didn't put it in becuase there bin is already made and couldn't go through each bin and sort out colors. But we will update it little at a time thats why we don't want to automatically assume someone should be white."* [[WH-35]] stops the warehouse *showing* an invented White; this clears the records that literally *store* one, because the portal bug above wrote `White` onto people who never picked it. **Customers → Bulk Updates → Wire colours nobody wrote down** reads her connected master sheet, lists every customer whose record says White while the sheet's Wire cell for them is empty, and clears only those — dry run first, then type `CLEAR`.
    - ⭐ **The sheet is the corroboration, and it is why no history archaeology is needed.** Field-level change logging only reaches back to 28 August while the portal bug shipped on 29 July, so there is a month with no log at all — but the Wire column covers every period, and it is the office's own record of what was actually chosen.
    - ⛔ **It only ever CLEARS, and only ever `White`.** The bug could not write Green, so a Green was typed by a person and is never touched; nothing in the sweep can set a colour on anybody. The worst it can do is ask a warehouse row to check the lights.
    - ⛔ **A customer with no sheet row is left alone.** An incomplete sheet is not evidence a record is wrong — only a row that *exists* with an empty Wire cell says the office recorded nothing. Those, and any the sheet and the record disagree about (record White, sheet Green), are counted and named but never cleared: that second kind is a real difference and **Compare** already offers it.
    - ⚠ **An ambiguous name is no match, not a weak one.** Names are matched through `dupNormName` (words sorted, so Last-First and First-Last are one key); two rows under one name resolve to nobody, or a shared name clears a stranger's wire.
    - ⚠ **It only ever writes what the dry run showed** — the delete step acts on the list already on screen rather than re-finding, so nothing she never saw can be touched. Same rule as the duplicate tools and the test-customer sweep.
    - ⚠ **The erasure half needs no new tool.** Where the portal bug *wiped* a wire colour instead of inventing one, the sheet still holds the real value and Compare already offers it as a difference — building a second importer for that would be a second opinion about the same column.
    - ⭐ **Measured before it was asked, off her own workbook**: 955 named people, **318 with a wire colour and 637 with none**. Of the 316 with a wire, 122 also have footage; of the 644 without, only **12** do, and **476** have no address or lights either — which is why WH-35 does not flood the build queue. It lands on roughly the 168 otherwise-complete rows missing only the wire, and "update it little at a time" is exactly what a *Check lights* row asks for, once, from somebody already holding the bin. `wire-sweep.test.js`, 23 checks that RUN the classifier, 9 sabotages red-checked.
    - ⭐ **And connecting the sheet now says how many PEOPLE it holds, not how many rows** (2026-09-16). Addie connected her workbook while this was being built and the line read *"Read 1058 rows straight from 2026 Client List (CURRENT - USE THIS ONE).xlsx."* She has **961 customers**. The other ninety-odd are the headings and the formatted-but-empty rows Excel keeps at the bottom of a sheet for ever — and a headline counting those invites the one reading that costs something: that the app cannot see a hundred people. `hlxWorkbookRowsAllSheets` returns a `people` count — rows carrying a **Name**, counted **after** the side sheets are folded in so somebody who exists only on Recycle is one of them — and `hlxSheetCountText` / `hlxSheetRowsNote` are the one wording both the Compare line and the first-connection line use. ⚠ **The raw count is kept, never replaced**: it is the half that says the whole file was read, so it moves into its own sentence and says what the difference is made of. ⚠ **And it falls back to rows when nothing counted people** — a workbook with no Name column returns `people: 0`, which is the wrong-workbook case that already has its own message naming the columns it could not find; *"0 people"* about a file that plainly read would be worse than the old wording. This is the same failure `hlxLoadConnectedSheet`’s own comment already records from the other end — *"924 rows"* reported confidently for the wrong workbook entirely. Suite 333, 10 sabotages red-checked; ⚠ one of them was **my own check being vacuous**: it asked whether `col["Name"]` appeared anywhere in the reader, and it appears half a dozen times, so a sabotage counting every row went straight through until the check was scoped to the counting loop.
    - ⛔ **And it reads whichever master sheet she has to hand, because the file route does not exist on her machine** (2026-09-17). Addie: *"The master sheet won’t add"* — and the screen was already telling her why: *"This browser cannot hold onto a file."* Connecting a FILE needs `window.showOpenFilePicker`, which exists in **Chrome and Edge on a computer and nowhere else** — not Safari, not Firefox, and **not on a tablet**, which is the machine this office has been caught out by before ([[MSG-05]]: the Inbox drag worked and there is no drag on a tablet). ⚠ **So the first version of the sweep was unreachable for her entirely**, and its refusal read like a missing step — *"press Use my master sheet above first"* — rather than a browser she cannot change. Compare has always accepted a **paste** for exactly this reason; the sweep asking for a file handle alone was an omission, not a rule. `wireSweepSheetRows` takes the connected file when there is one and the **Paste the whole sheet** box when there is not, finding the headings wherever they sit (a sheet can open with a title line), and the report names which of the two it read — a stale paste and a live file must never look identical on screen. ⚠ **A paste is the weaker source and it fails safe**: it carries one tab, so somebody who exists only on Recycle has no row, and no row means LEFT ALONE, never cleared. A partial paste is the same shape — fewer people reachable, none wrongly cleared. ⛔ **But a Wire column that arrived EMPTY is refused outright**, and that is the one case that could have done real damage: hidden, filtered out, or a copy that stopped short all give a Wire heading with nothing under it, and on that reading every customer with a row looks like somebody the office never recorded — so the sweep would clear **every real White in the book**. Her sheet carries 318 wire colours, so a sheet with none is not one to act on. ⚠ And the checks for all of this **ran after the summary had printed** in their first draft — eight of them, unable to fail the build, the trap this file names about Suite 10. They are awaited now. 7 sabotages red-checked.
  - ⭐ **AND THEN NOBODY IS ASKED FOR A WIRE COLOUR AT ALL** (2026-09-17, [[OPT-12]], superseding [[OPT-11]] — ⛔ **and itself superseded the next day by [[OPT-21]] for the QUOTE FORM half; the portal half below still stands**). Addie, once it was clear she cannot connect the master sheet on her machine: *"keep what lights they want but don’t add what wire color they want but push check lights then warehouse chooses what wire they have on file and will make it based on what wire they have."* ⛔ **FIVE DOORS WERE INVENTING A COLOUR, NOT ONE**, and the portal dropdown everything above is about was the smallest of them. The public quote form defaulted to `Any`; `quoteSaveDetails` stamped the same `Any` on the SERVER, which is the emailed-link path and the common one; **Add Customer and Edit Customer both opened on White with no blank option at all**, so every customer the office added was recorded White whether or not anybody had been near their bin; and the conversion prefill fell back to White whenever a quote carried nothing. That is why [[WH-35]]’s *Check lights* could almost never appear for a new customer, and it is the real reason the book is full of Whites. ⭐ **The light colours are untouched** — her sentence begins *"keep what lights they want"*, and only the cord is ours. ⭐ **Blank is a real answer now**: both office selects open on *— not chosen, warehouse will check —*, there is no default and nothing is required, so a wire colour exists only where somebody deliberately set one. ⛔ **And the removal is enforced rather than merely unwired**: `wireColor` is out of `PORTAL_WRITE_FIELDS`, so the public site cannot write it even if something tried, and out of `PORTAL_READ_FIELDS`, so the browser is never handed one. It is `internal` in the option registry — R-003’s own stated exception — which is what takes `quote` and `confirmation` off its consumer list and changed the frozen AGREED map for the first time since it was agreed. ⚠ **Nothing stored is rewritten.** A wire colour already on file still prints and still groups; what changed is that no new one is invented. ⚠ **And a Green house needs no checking** ([[OPT-13]]) — recorded, not built, because it was already true: nothing in the app has ever written Green by itself, so a Green was typed by a person and is evidence where a White is not. ⚠ `test/wire-colour.spec.js` is REWRITTEN rather than repaired — the control it drove is gone, which is the quote-card.test.js precedent — and the claim it could not make (that the browser is never even told) is handed to `portal-fields.test.js`, which can see the whitelist. Two drafts of that check were vacuous first: one read a call log that records no result, the other read a `var` that never reaches `window` because index.html’s portal script is one ES module.
  - ⭐ **AND THE SAME QUESTION IS ON THE INSTALL-DETAILS FORM** (2026-09-18, [[OPT-22]], the other door to [[OPT-21]] above). Addie: *“They should see Any, Green, White. With instructions on what to pick. If they click any or keep it at any then it should allow us to pick and require us to pick the wire.”* [[OPT-21]] put it on the member portal; this is the same three answers on the form somebody who is **not a member yet** fills in after approving — three pills in her order, **Any** already picked, **and the instruction copied from OPT-21 word for word** (*matches your gutter… green gutters take green wire*). Two doors telling somebody different things about one choice is how they start answering differently at each. ⛔ **“Any” stores nothing at all, and that one line is the whole design.** It is not a colour — it is her saying she has no preference, and the way to record that is to record nothing. A blank is already what the rest of the app understands: the warehouse heading reads **Check lights** ([[WH-35]]), the **Pick a Wire Colour** System Message with a picture of the house is raised ([[WH-40]]), and the **Convert to Customer** popup will not convert until the office picks one ([[WH-43]]) — which is the second half of her own sentence, already built the same day. ⚠ **Both narrow [[OPT-12]], and its argument is what makes them safe.** That ruling was refusing an **invented** answer: the old control defaulted to `Any` and **stored the word**, so most quotes carried a cord nobody had chosen. The question was never the fault. ⛔ **Absent, not blank.** Neither the browser nor the server writes `wireColor: ''` — a posted blank would **erase** a colour a re-quote had prefilled off the member's own record. ⚠ **Two copies of one rule, compared**: `qdWireChoice` in index.html decides what the radio posts, `quoteWireChoiceServer` decides what is stored (that callable is reached with nothing but a quote token, so the browser's answer is a suggestion), and `wire-pick.test.js` runs both side by side. ⛔ **The portal half is [[OPT-21]]'s** — its own control on the lights tab, its own spec, and nothing here reads or changes it. The **Changes** tab still has no wire control at all. ⚠ **And it puts back a property the White sweep already leans on**: `wireSweepClassify` keeps a White that came from a quote *because* that form defaults to Any, so a White on one is a customer ticking White. That sentence stopped being true when the control was removed; it is true again. `test/quote-wire-choice.spec.js` drives it in a real browser, because every claim here is a control on screen or a field in a save.
  - ⭐ **AND THE LIST ADDIE READS WAS STILL SAYING THE OFFICE PICKS IT** (2026-09-19, [[OPT-26]]). Addie, told the options list had not caught up: *"Fix the options list to say customers can pick it."* ⛔ **NOTHING WAS WRONG WITH THE APP, AND THAT IS THE WHOLE POINT OF THE ENTRY.** `internal` came off the `wireColor` entry with [[OPT-22]], so the REGISTRY had said for a day that the customer picks it, and every machine-derived half of `docs/what-we-can-record.md` — the destinations, the price line, the count — was already right. What was wrong was the ONE HAND-WRITTEN half: the *Asked at* sentence named a single door and said the office picks when they said Any, and the page's two groups (**What the customer tells us** against **What we work out ourselves**) were derived from **that sentence** rather than from the flag. One claim, two sources, and the typed one was winning. ⭐ **The grouping reads `internal` now** — the half with a gate behind it, since `audit()` refuses a non-internal option that no customer-facing artifact renders — and the generator **refuses to build** when the flag and the sentence disagree, in either direction, because an option marked internal whose prose says the quote form asks it is either a leak or a stale line and only a person knows which. ⛔ **And the committed page is compared against the registry on every run** (`node docs/build-what-we-can-record.js --check`, run by `options-audit.test.js`): *"Generated from js/options.js"* was only ever true of the RUN, never of the copy on disk, so that promise rested on somebody remembering one command. ⚠ **That promise is checklist row 216's, which is why it earned code.** That row sends her to this page to answer the one question no test can — *is anything sold today that has no row at all* — and tells her it *"cannot be out of date with what the software does"*. A stale copy there is not an untidy document; it is the wrong answer at the one moment she is checking by hand. ⚠ **The check writes nothing**: a gate that repaired the file would hide the drift it exists to report, and would edit the working tree during a test run. ⚠ **Nothing is reversed** — [[OPT-21]] and [[OPT-22]] both stand, and `confirmation` stays off `wireColor`'s consumers as the declared gap it already was. ⚠ **A stale claim went with it**: the `wireColor` comment in `js/options.js` still read *"`wireColor` is still out of PORTAL_WRITE_FIELDS and PORTAL_READ_FIELDS"*, written to scope OPT-22 to the quote door while OPT-21 was merging from a parallel session — and that ruling put it back on **both** lists within the hour. ⚠ **The portal is still not one of the eight destinations**, so it cannot be named in `consumers`; where a customer is ASKED is carried by the prose, which is exactly why the prose now has a gate on it.
    - ⭐ **And every stored White is swept, not only the uncorroborated ones** (2026-09-17, [[OPT-14]]). Addie: *"can we sweep all whites but in the future if they do save that they want white wire we will save it for [them]."* [[WH-36]] cleared a White only where the master sheet held a row with an empty Wire cell — a corroboration rule written while the belief was that ONE door had invented the colour. **Five had**, so a stored White is not evidence of anything and there is nothing left worth corroborating. ⛔ **Green is still never touched** ([[OPT-13]]); that asymmetry is the whole argument for why sweeping every White is safe and sweeping every wire colour would not be. ⭐ **The sheet is optional now** — it decides nothing, and only says how many of the cleared ones it happens to agree with, so the tool needs no file handle and no paste. That is what finally makes it work on the machine the office has. ⚠ The breakdown is still worth printing: the count the sheet AGREES with is the honest measure of what the sweep costs, and those are recoverable — **Compare** puts the sheet’s own answer back. ⚠ An empty Wire column no longer refuses the sweep, but is reported as *no comparison* rather than counted as agreement. ⚠ And only the exact word, trimmed: *Off White* is somebody’s own note.
    - ⭐ **And then only the ones NOBODY CHOSE** (2026-09-17, [[OPT-17]]). Addie, shown what a full sweep would take: *"Everyone should not have there whites sweeped. Quotes should definetly keep what they put on the form."* A White from their **quote**, a **re-quote** or the **Member Portal** is kept and named on screen; everything else goes. ⭐ This narrows the bullet above **on the same evidence that widened it**, and she is right: *a stored White is worthless as evidence* was true of the doors the OFFICE invented it at, and never true of a quote — that form’s default was `Any`, on the browser and the server alike, so a White on one is a customer ticking White. ⛔ **Nothing ever recorded where a wire colour came from** — the wall `lightsChangedVia` had to be built to get over. But the trail already exists in two collections: every quote and re-quote names its customer (`existingCustomerId`) and still holds the colour picked, and the portal wrote an Inbox message (`Wire Color Change`) on every change. So `wireSweepSources` reads records **already filed** rather than needing a new field — which is the only reason it can reach the houses that are already wrong. ⛔ `Any` is not a choice; it was the default, which is the customer not answering. ⚠ An ambiguous phone or email **keeps every household it could mean**: a kept White is visible and clearable later, a wrongly cleared one costs a trip through Compare. ⚠ And one caveat said out loud: the portal invented White too ([[OPT-11]]) and that invention wrote the same message, so a few kept ones are the bug rather than a choice. Kept anyway — the safe direction, and what she asked for.
    - ⛔ **And the Member Portal is NOT a second reason** (2026-09-17, [[OPT-18]], superseding [[OPT-17]] after one commit). Addie, once the portal half had been built and its one weakness was put to her: *"just get rid of all whites then unless its a quote or requote."* She took the caveat rather than the feature. **The portal invented White too** ([[OPT-11]]) — its dropdown prefilled the colour for anyone with nothing on file — and saving that wrote the very same `Wire Color Change` message a real choice writes. So the portal trail cannot tell an answer from the bug, and keeping on it would spare records nobody chose **while calling them chosen**, which is worse than sweeping them. A **quote** can tell, because its default was `Any`. ⚠ The message scan is **gone rather than unused**, and that is asserted on `wireSweepSources` rather than on the classifier: the red-check proved that putting the branch back into the classifier alone is a NO-OP, since nothing supplies the key — so the check has to refuse the Inbox read itself. ⚠ And **both spellings of the quote link** are read, `existingCustomerId` and the older `convertedToCustomerId`, or every customer converted before that rename is swept in spite of their quote.
    - ⭐ **And only the houses IN THE WAREHOUSE right now** (2026-09-17, [[OPT-19]]). Addie, reading a dry run that listed most of the book: *"it looks like its everyone that has whites that we would loose there whites. I only want people that are currently in warehouse with white lights to be refreshed."* ⭐ **The reason is the cost, which is what makes it the right scope rather than merely a smaller one**: a wrong wire colour only matters to somebody about to MAKE a bundle — those are the houses where *Check lights* gets answered, because a person is already at the shelf. Clearing the other ~900 asks a question nobody is in a position to answer, and empties records that were at least a guess. ⚠ So this is **a habit, not one sweep**: a house leaves the scope by being built, so the same button run next week catches whoever is queued then. ⛔ `wireSweepOnWarehouse` asks **`whBuildQueueGroups` — the tab’s own answer** — rather than testing `needsLightBuild` for itself: a second opinion about who is in the warehouse is how this sweep and the screen it is named after start disagreeing, which is the fault this whole thread has been about. ⚠ Called **once**, not per house — `whHouseBuildStatus` answers it for ONE house by building the whole queue, and asking that per customer is the ~950,000-comparison shape that has already locked a screen in this app. ⚠ The **blocked block and both timer lists** count as in the warehouse; buffer stock does not, because an extra has no customer record to clear. ⛔ And a **missing map sweeps nobody**, not everybody — failing the other way would look exactly like the sweep working.
    - ⛔ **And a fifth door was found by checking the answer rather than quoting it** (2026-09-17, [[OPT-20]]). Addie: *"So for the future the only people that are marked for white wire will have marked it themselves or we marked it correct?"* Running the census to answer her, rather than repeating what it claims, showed that **`employee.html` is not in it** — its `FILES` list is admin.html, index.html and functions/index.js. The crew portal carries its **own copy of `whWireLabel`**, and that copy still returned **White** for a blank while the office copy has said *Check lights* since [[WH-35]]; one render there skipped the label entirely and printed the raw field. ⛔ **The cost is not just a wrong word on a screen**: that label is **half the build group key** — a build is only "the same" if the colours AND the wire match — so the office filed blank-wire houses under *Check lights* while the crew portal filed them under *White*. Two screens, two piles, one house. ⚠ **And the gate lifted the crew copy as a dependency and never compared it**: the colour comparison hands `whWireLabel` to its sandbox and then only exercises the colour half, so every check stayed green through the drift. Both copies are now RUN over the wire half, the blank included — and the blank is asserted outright, not merely that the two agree, because two copies can agree on White and both be wrong. ⚠ **Dormant is not harmless**: the crew portal is unused this season, which is the same argument `silent-failures.test.js` already makes for sweeping it. Rules 1 and 2 of the census both cover it now.
    - ⛔ **And four MORE doors were still writing or printing it** (2026-09-17, [[OPT-16]]). Addie: *"I still see that white wire is still showing in warehouse."* Two things were true at once and only saying one would have been wrong. ⛔ **`openEditCustomerModal` filled the box with a White fallback**, and that form is opened dozens of times a day — so every save of a blank-wire customer wrote White back, and **a sweep would have been undone one save at a time, silently.** The others: the Add Customer form RESET opened on White; `renderRouteOrderedList` printed *Wire: White* for a blank; and the printed Needs Building sheet printed an EMPTY cell where the screen has read Check lights since [[WH-35]] — and a blank on paper says nothing when the one thing that row needs to say is go and look. ⚠ **The warehouse itself was right all along**: its chips, build groups and both sheets already went through `whWireLabel`. What she was looking at was records that really did say White, because the sweep is a button and nothing had pressed it. ⭐ **So the rule is CODE now rather than a habit** — §6’s own promotion rule, after finding these one at a time by eye over two days. The census in `warehouse-colours.test.js` refuses any `|| 'White'` in any source file, and requires every place a stored wire colour reaches a person to go through `whWireLabel`. ⚠ It is **named, never counted**: a ceiling on raw reads is the gate this repo has rejected twice. ⚠ And its first draft sat after `process.exit` and never ran — all five red-checks reported MISSED and the gate was worth nothing; the tell is a new section that passes the instant it is written.
    - ⭐ **And switching the member auto-reply on is one press** (2026-09-17, [[OPT-15]]). Addie: *"I already gave you a template to work with."* She had, twice — and being walked through *press this, then tick that, then Save* is the complaint. **Use the wording you gave us, and switch it on** creates the template, picks it, ticks the box and saves in one press. ⛔ **The press stays hers**: seeding it on page load would mean real customers start getting email because a page rendered. The tick box turns it straight back off. ⚠ It never overwrites wording she has edited — an existing template of that name is switched on as it stands. ⭐ And her closing *"This is auto- Reply"* was a **label, not copy**: the second paste of the same wording did not carry it, so the grey footer is gone and a check stops it coming back.
  - ⭐ **AND THE UNANSWERED QUESTION IS ANSWERED IN SYSTEM MESSAGES, OFF A PICTURE OF THE HOUSE** (2026-09-17, [[WH-40]]). Dax: *"if they dont answer it then there is a spot in our system messages that shows us a picture of their house and we can mark there wire type there and there is no default color so they dont get assigned a color until we go into system messages and assign them one."* ⛔ **“Check lights” is the right answer for a bin and the wrong one for a new house.** After [[OPT-12]] nobody is asked for a wire at all and the office form opens on *not chosen*, so every new house arrives carrying nothing — and [[WH-35]] then tells the warehouse to look in a bin that does not exist yet for a house that has never been hung. A **Pick a Wire Colour** notice is raised the moment a customer is saved with the wire genuinely blank (only that door, only when blank), carrying their `customerId`. The card shows the photo on their record, or Street View worked out from their map pin, and **White** / **Green** write straight onto the record.
    - ⚠ **The answer is read off the CUSTOMER, never off the notice.** The wire can also be set from Edit Customer, the member portal or the sheet sync, so a card that believed its own buttons would go on asking for something already answered — and pressing it would write over a colour a person had since chosen. A card whose customer has been removed says so and draws no buttons at all.
    - ⚠ **At creation only — never a sweep over the book.** ~637 customers have no wire on file and are deliberately left to be answered at the shelf, which is [[WH-36]]'s own *“we will update it little at a time”*. A notice each would bury the folder this one has to be noticed in.
    - ⚠ **It queues a rebuild, through `warehouseRebuildFields` rather than by setting the flag itself**, and dates it with `stampBuildQueued` like every other door that queues one — a house already built in the Check lights pile has to be built again now the wire is known. A refused customer write leaves the notice OPEN: a closed notice over a failed write is the one outcome where the question disappears and the answer never lands.
    - ⚠ **Two Google keys, and only one of them has Street View turned on.** The picture walks the same key list `rmFetchStatic` walks; a refusal tries the next key, while *no imagery here* is answered once and said plainly, because it would say the same on every key we own.
  - ⭐ **AND IT IS ASKED AT THE CONVERT TOO, BEFORE THE CUSTOMER EXISTS** (2026-09-18, [[WH-42]]). Addie: *“when they choose any or they don't choose any they just leave it at any that it notifies us when we push convert to costumer before it converts it will come up with a pop that says pick a wire color for costumer or something similar to that.”* Pressing **Convert to Customer** already opens a popup (the one offering *Convert automatically* / *Fill in manually*, listing what the quote is missing). It now carries a **Wire colour for this customer** picker, above the two buttons, and whichever button is pressed the answer lands on the new customer's record.
    - ⛔ **It adds a door and closes none of [[WH-40]]'s.** No default colour, the System Message still raised for a house left unanswered, the warehouse still reading *Check lights* ([[WH-35]]). What was missing is that the question was only ever asked AFTER the customer existed — the one moment somebody is already looking at that house was the one moment nobody was asked.
    - ⭐ **AND IT IS REQUIRED** (2026-09-18, [[WH-43]]). Addie, told it only asked: *“make it required for us to choose wire before we convert to costumer.”* Both buttons — *Convert automatically* and *Fill in manually* — are greyed out until a wire is picked, each saying why, and the handlers refuse again on the press because a greyed button is still reachable by keyboard. A refused press leaves the popup up rather than closing on nothing.
    - ~~⚠ **It asks, it never refuses.** *Not now — the warehouse will check* is a real answer and leaves today's behaviour exactly as it was. There is often genuinely nothing to answer from yet, which is [[WH-40]]'s own reasoning; a guard that blocked the conversion would have no way round it.~~ ⛔ **Superseded the same day by [[WH-43]].** The struck reasoning is kept because it is the cost she accepted, not a mistake: a house with no picture and nothing to check has to be left and come back to. **Cancel** is the way out.
    - ⛔ **The popup refuses; the Add Customer form does not.** That box stays optional, and its blank-wire branch is the only thing that raises [[WH-40]]'s System Message — requiring a wire there too would make that notice unraisable, which is WH-40 deleted by accident through a different ruling. A customer typed in from scratch is not a conversion. Edit Customer and the re-quote path are untouched for the same reason, and because two thirds of the book ([[WH-36]]) has no wire on file: requiring one to fix a phone number would lock ~640 records.
    - ⛔ **The blank option stays in the list, relabelled “— pick one —”.** Dropped, the select would open preselected on the first real colour and one press would save White onto a house nobody looked at — the invented colour [[WH-35]] and [[WH-40]] both exist to stop, arriving by accident through the very change meant to make somebody choose.
    - ⚠ **The options are read off the Add Customer select**, never typed into the popup — add a wire colour there and the picker offers it on the next render.
    - ⚠ **Both convert paths apply it AFTER `fillAddCustFromQuote`**, which sets that box from the QUOTE. Applied first, the quote's own blank overwrites the answer and the picker silently does nothing — and blank is a legal value here, so it would never throw.
    - ⚠ **The automatic path names the wire in its toast**, because that path never shows her the form: without it the one thing she was just asked for is the one thing she cannot see took.
  - ⛔ **And a second one, older and worse, in the portal's wire-colour dropdown.** It was prefilled with `addrDoc.wireColor || 'White'` and read back on save with no fallback — and `wireColor` is one of the three `WAREHOUSE_BUILD_FIELDS`, so whatever that control reads can queue a bundle rebuild. Two silent faults, both measured by running `warehouseRebuildFields`: a customer with **no** wire colour on file (`rbNormalizeWire` returns '' for anything the sheet spelt oddly) who opened the Changes tab to edit their **note** had `White` written to their record — a colour nobody chose and a rebuild nobody asked for; and a record holding a wire the dropdown cannot show fell to `selectedIndex -1`, so the save wrote `''` and **erased it**, rebuild and all. The radios directly below that select already avoid exactly this — an untouched control must never read as a change — so the select now follows the same rule: a disabled *— not recorded —* option when there is nothing on file, and a save that falls back to the record rather than writing a blank over it. ⚠ Covered by `test/wire-colour.spec.js` in a real browser, because the whole fault is what a `<select>` does with a value that is not one of its options — something no source check can see.
  - ⛔ **And building it found a real bug in the office history, fixed in the same push.** `specificOutlet` is a Yes/No field, and the rule that reads those answered `v ? 'yes' : 'no'` — but the portal's radios post the **string** `'No'`, which is truthy. So it was wrong in both directions at once: a customer switching *Specific outlet* from No to Yes produced **no history row at all**, while a record holding boolean `false` saved against a posted `'No'` — nobody touching anything — reported *“no → yes”* every single time. On the old code that was a wrong line in a log; on the new code it would also have been an email to a customer about a change they never made. Blank, `no`, `false` and `0` all read as no now, and anything else as yes — which keeps the older trap closed, because `''` and `false` still have to mean the same thing or every save reports its tick boxes changing. ⚠ Paired rule: `portalChangeValueText` in functions/index.js and `changeValueText` in admin.html, changed together.
  - ⚠ **Sides promise nothing about the price.** Changing them raises a re-quote, so that line says an updated price is coming rather than letting the template's closing sentence promise the work at the old figure.
  - ⚠ **The same change twice inside half an hour is one email**, fingerprinted on the wording rather than on which fields moved — so a red-then-blue correction still gets its own email, because a stale confirmation naming the colour they backed out of is worse than a second one.
  - ⚠ **The template is loaded by id, and a deleted one sends nothing** rather than wording she never wrote. If the body loses `{{change}}`, the list of what changed is appended anyway — the email can never fail to say what it is about.
...
- **Only people who answered yes are scheduled or invoiced** (see §1 step 7, questions map RS-15, RS-17 and RS-22). Waiting does not turn an unanswered customer into a yes. ⛔ **AND THERE IS NO LEVER AT ALL — corrected 2026-09-19.** This line used to end *"the only lever is whether the RSVP has been marked sent"*, which stopped being true on 2026-08-27: the rule applies ALWAYS ([[RS-22]]), and `rsvpSentAt` decides nothing about who gets a crew. It is still worth setting, because it is the only thing separating *they have not replied* from *we have not asked them* — which is what makes **Schedule › Waiting on RSVP** actionable. ⚠ **That stale sentence had a cost**: checklist row 215 told her to press the season switch and confirm it refuses until the RSVP is marked sent, and it has not refused for three weeks. Fixed at v2 the same day this line was.
...
- Every morning at 8, `cloudinaryUsageWatch` reads how much of this month's Cloudinary credits are used. At 80%, and again at 95%, it leaves one **Photo Storage Needs You** note in the Inbox. If Cloudinary has switched the account off, it leaves one that day. That is what happened on 9/9, when uploads failed with "cloud_name is disabled" and nobody had been warned ([[PROC-34]]). Every photo on a quote or crew sheet lives in Cloudinary.
...
- **Sending the texts through a bulk tool** (Automation Emails → RSVP → *Text the RSVP*, 2026-09-21, [[RS-67]]). Addie: *"I'll send out all texts through google workspace bulk text."* Three buttons on that card, and they answer three different questions.
  - **Copy pending phone numbers** (was *Copy every phone number* until 2026-09-24, [[RS-72]]; it never copied everybody) — the recipient list of people still to answer, one per line, digits only with any leading 1 dropped so the tool sees one shape. Anybody who replied **STOP is left out**, and the line underneath says how many. A number that is not 10 digits goes in exactly as it is and is named, so an odd one can be looked at rather than silently lost.
  - **Who has no number** — the people a text can never reach, listed on screen with their email and address, and copied as a sheet. `"n/a"` and `"none"` in the phone column count as no number. Anybody with an email is still in the RSVP email; anybody with neither has to be rung.
  - **Copy everyone's links — for a spreadsheet** — one row per person: Name, Phone, Email, Do not text, Their RSVP link, Their referral link, **Text message**. The message column is the same wording the Copy button produces, ready for a mail merge or a bulk send.
  - ⚠ **STOP means two different things in those two places, deliberately.** On the numbers list it is an exclusion, because that list is pasted straight into a sender and a marker nobody reads protects nobody. On the sheet they are kept, marked **DO NOT TEXT**, and sorted to the very bottom — the sheet is read, and dropping them would lose them from the one list meant to reach everybody.
  - ⚠ **The referral link is a column, never part of the text.** A second address takes the message past one 160-character segment, and two links in front of somebody with one question to answer is what the /a/ page exists to avoid. They get the referral offer on the page they land on.
- **Copy their phone numbers, straight off the email filters** (Automation Emails → Preview & Send recipient list, 2026-09-24, [[RS-68]]). Addie: *"filter out pending numbers on automation email then RSVP then text the RSVP."* Set **RSVP Status → Not answered yet**, send the RSVP, then press **Copy their phone numbers** beside Select everyone matching and paste the column into the bulk texter. It copies **whoever the filters match** (not the ticks, not the search box), digits only, one per line — including people with **no email**, whom the email just skipped, and it says how many of those there were. STOP replies, the do-not-send list and anybody with no number are left out and each is counted on the line underneath. Numbers only: the per-person RSVP links are on the Text the RSVP card's link sheet.
  - **A batch at a time** ([[RS-69]], same day). Both copy buttons — this one and Text the RSVP's *Copy pending phone numbers* — now open a panel: **How many at a time** (default 50), **Separate with** one per line (the default since [[RS-73]] — a column, not a paragraph) or commas, the current batch shown in a box, and **Previous batch / Copy batch N / Next batch**. Opening the panel copies nothing — only **Copy batch** does ([[RS-72]]). If the browser refuses the copy the numbers are left selected in the box and the line says press Ctrl+C. The size and separator are remembered on that browser — the size is saved as she types it and applies straight away, and a blank box goes back to the saved number ([[RS-71]]). There is no cap: the batch is whatever she types. ⚠ **Copy their phone numbers** on Automation Emails copies whatever the filters match, so it warns when **RSVP Status** is not *Not answered yet* — on All it is the whole book ([[RS-72]]).
  - **The batches are kept, and each one has a Sent tick** ([[RS-75]], same day). Addie did not want the list re-cut from whoever is pending each time. This replaced [[RS-74]] from earlier the same day, which left anybody already copied out of the next opening — that re-cut the list too. The first press saves the list (`settings/phoneBatchRun_rsvp` for Text the RSVP, `settings/phoneBatchRun_et` for Automation Emails) and every later press reopens that same list, on any computer, at the first batch not yet ticked. Each batch has an **I sent batch N** tick, saved with when and who, and a row of batch numbers shows which are done (✓, green). Somebody who answers after being batched stays in their batch. People who become pending later are counted and offered as **new batches at the end**, never slotted into a batch already made. Once any batch is ticked the size is locked; **Start a new list** rebuilds from who is pending (or what the filters match) now, and asks first when ticks would be lost.
  - **Texted people stay off new lists** ([[RS-78]], same day). Ticking a batch **Sent** also stamps `rsvpTextedAt` on every customer on those numbers, so **Start a new list** (and either copy button) leaves anybody texted this calendar year out and says how many. Under the batches, *Already texted some of these?* takes a paste of numbers, or **I texted everyone on this list up to and including <name>** — the list is alphabetical, so that marks everybody through that person (anybody on Unanswered excepted, since an earlier list did not hold them). With nothing ticked yet, the list is made again straight away without them.
  - **Names next to the numbers** ([[RS-70]], same day). An **Include names** tick box, on by default: number first, then the name (`8015550111 Ann Smith`), because her paste box is labelled "Numbers, names". Number, a space, then the name, in both layouts ([[RS-73]]). A comma in a name becomes a space so one person cannot split in two. Untick it for bare numbers; remembered on that browser.
- **Everyone's own two links, as a spreadsheet** (Automation Emails → RSVP → *Text the RSVP* → **Copy everyone's links — for a spreadsheet**, 2026-09-21, [[REF-44]]). Addie: *"is there any possible way I can send everyone there own link and there own referal link?"*
  - ⚠ **Not from one copied message, and this is the part worth understanding.** Each RSVP link carries that customer's own token — it is what signs them in and records their answer — and each referral link carries the token that credits their $25. One message pasted to everybody would sign every customer into the same account and credit every referral to the same person.
  - **So there are three ways to get everyone their own, and all three already work:**
    - **Preview & Send** sends one email per customer in a single press, each with their own links filled in. Nothing to copy at all.
    - **A mail merge** — press this button, paste the sheet into Google Sheets or Excel, and use a Gmail mail-merge add-on. Still one copy and one paste, but the merge sends a personalised email per row.
    - **One at a time** — *Copy the finished email* on Preview & Send, which is right for a handful.
  - **What the sheet holds:** Name, Phone, Email, Do not text, Their RSVP link, Their referral link — tab-separated with a heading row, one row per customer still waiting to answer, sorted by name.
  - ⚠ **No phone is needed to be on it**, unlike the texting list on the same card: an email does not need one, and requiring one would drop exactly the people this is for. Anyone who has already answered, and anyone in their first year, is still left out.
  - ⚠ **It makes a link for each person, so it writes to each of their records** — that is why it names the count and asks before it starts, and why it can take a moment on a big list. If any row comes back without one of its two links the line underneath says how many.
  - ⚠ Somebody who replied **STOP** stays on the sheet with **DO NOT TEXT** in its own column. STOP means do not text them; it says nothing about email, and dropping them would lose them from the one list meant to reach everybody.
- **Sending one by hand: copy the finished email, not the template** (Automation Emails → Preview & Send, 2026-09-21, [[EM-24]]). Addie: *"I don't have a automation to go through so I just have to copy and paste it."*
  - **Pick the template, pick somebody in *Preview Against*, then press *Copy the finished email*.** That is the copy with the codes already filled in. Copying out of the **Body** box in the editor above gives you the template — the version that still says `{{rsvp_link}}` — and that is what arrived in a real customer's inbox on 2026-09-21, reading `{{referral_link}}` and `{{rsvp_link}}` exactly as typed.
  - ⚠ **Neither code was broken**, and it is worth knowing before anybody goes looking: every sender fills them in, and a code with nothing behind it comes out **empty**, never as its own characters. Characters mean the text never went through a sender at all.
  - ⚠ **The links belong to the member in *Preview Against*.** Each one carries that person's token, which is what signs them in and records the answer against them, so one copied email cannot be sent to the book. The status line names whose copy it is.
  - **It pastes into Gmail with the buttons intact**, because the clipboard carries the HTML as well as the text. Anywhere that cannot take HTML gets the text version, where each address is written out after its own label — `Answer here: https://…` — rather than the label alone. It used to copy the words and drop every address, so a pasted RSVP said Yes / No / Back Next Year with nothing behind any of them.
  - ⚠ **If anything could not be filled in, the line under the button names it.** A code nothing knows about is copied through as itself, which is the very thing this was reported for.
...
no colours, so every customer it makes lands in the build queue's **Check lights** group
([[WH-41]] — a *Waiting on light colours* block until 2026-09-18), which is visible and has
an Edit button. Gating the flag would make those houses invisible instead, which is the bug
that ruling closed.
...
- **The Dashboard says people are waiting, but there is nothing to press** → that is correct, and it is not a missing button. `SEASON_ELIGIBILITY` is a **constant** — the rule that only people who answered Yes are scheduled is always on, nothing in the app can turn it off, and the row beginning *Who the crews are sent to:* is read-only and says so. The work is **Schedule › Waiting on RSVP**: everybody who has not answered, by town, with a phone number. Marking an answer on their record moves them off that list and onto the routes by itself. ⚠ **If a button ever DOES appear on that row, treat it as a fault** — something has re-added a lever Addie removed on 2026-08-27 ([[RS-22]]), and it can empty a season in one click. ⛔ **Checklist row 215 told her to press three such buttons for three weeks**, and a same-day "fix" corrected only one of the ten steps; the labels are in `RETIRED_CHECKLIST_TERMS` now so no future row can name them, and the row is v3.

- **The admin page keeps going unresponsive, and a refresh does not come back** → look for a listener that DRAWS on every snapshot. Fixed 2026-09-18 (Dax: *"the page keeps becoming unresponsive"*, *"when i refresh it never refreshed"*). The invoices listener redrew the invoice list, All Customers, the route list, takedowns, the Dashboard and Expenses, and ran two payment sweeps, on EVERY invoice write, with no batching. A burst of a few hundred writes (the sync at load makes one) queued a few hundred full redraws. And each redraw of the invoice list re-grouped all ~957 customers once PER ROW (`billingGroupsByPayer`, through `invoiceDisplayName`), about a million steps per draw. Measured on a full-size fake book, the load froze the page for **271 seconds** before the fix and about **8 seconds** after, with no single block over half a second. What changed:
  - ⭐ **The data is updated on every snapshot and the screens draw once per burst.** `loadInvoices` rebuilds `allInvoicesCache` and `invoiceById` straight away, then `scheduleInvoiceDraw` waits for 250 ms of quiet, or at most 2.5 s. These are the same numbers `scheduleJobAddressRender` has always used for customers. The Inbox (`loadMessages`) got the same treatment. `flushAdminErrors` still runs on every messages snapshot, because the error reporter must see each one. ⚠ The last write always draws. Nothing is left stale.
  - ⭐ **All Customers, the route list and takedowns go through `safeRender` now**, so a panel that is not open is drawn when it is opened instead of on every payment.
  - ⭐ **`billingGroupsByPayer` is built once per draw of the invoice list, not once per row.** `billingGroupsMemo` only exists while `renderInvoicesList` is running (its `depth++ … finally depth--`) and is dropped the moment the draw ends. ⚠ Everywhere else, including every save path and every test sandbox, the grouping is computed fresh exactly as before, so a record changed in place is never read stale.
  - Suite 343 in run-all.js RUNS both listeners against a fake snapshot source and counts draws. Three sabotages were red-checked and all three were caught.
  - ⚠ **If it comes back**, the next suspect is another listener that draws on every snapshot. Any listener that writes a screen should coalesce like these do.
  - ⭐ **AND IT DID COME BACK, FROM A DIFFERENT PLACE (2026-09-19).** Dax: *"switch to customers then all customers ... and then refresh"*. The page stopped answering, and the customer count read **0** while all 957 customers were safe in the database (counted directly). The count was 0 only because the page froze before drawing it. **All Customers asked `planHangDateFor` once or twice per row** (the Route status column and the next-visit chip). Every call walked the whole season plan through `schedulePlanBookings`, working out each day's crew towns again with `dayCrewTowns` / `bestCrewTowns`. Measured on the real book, the click was still blocked after **90 seconds**; after the fix it answers in about **0.25 s**. A refresh never helped because the page reopened on the same tab. The listener fix above could not reach this: it was one synchronous draw, not a burst of writes.
    - ⭐ `planHangDateFor` keeps the plan's answer until the current synchronous task ends (cleared on the next microtask). The table, its filter and the Member Export each read the plan once per pass, and the next draw reads it fresh. It also remembers which `schedulePlanBookings` answered, so a swapped publisher is never handed the old answer.
    - ⚠ **Display only.** Nothing that writes reads `planHangDateFor`, which is what makes a task-long memo safe. Do not route a write path through it.
    - ⚠ **This was found only by running the page against a read-only copy of the REAL data and clicking the sub-tab.** The earlier harness opened the Customers tab but never clicked All Customers. On a fake book, the season plan has no days worth walking.
    - ⭐ **The Inbox had a smaller version of the same shape, seen in a profile of the live page.** The sidebar counts every tab by running every message through `msgFacets`, and `msgCategories` runs about a dozen text patterns over the message body. With 314 real messages that was ~2 s of frozen screen per sidebar draw. `msgFacets` now remembers its answer against the message object, keyed on EVERY field it reads (topic, folder, archived, status, responded, read, priority, important, severity, tags, message), so a message marked read in place is seen at once. Measured on the real messages: 2.4 s blocked → 0.6 s across the same six Inbox clicks. ⚠ If a facet function starts reading a new field, add it to that key. Suite 345; three sabotages red-checked, all caught.
    - Suite 344 RUNS the function against a counting publisher: 957 rows ask the plan once, the next draw asks again, "cannot say" still reads null, and a swapped publisher is asked afresh. Three sabotages were red-checked, all caught.
...
⭐ **Added 2026-09-12.** Three separate things get called "Google" and only one of them costs money. They are listed here because the confusion is the expensive part.
...
⭐ **The site is SEVEN URLs now, not one** (changed 2026-09-12, later the same day). The line here used to read *"the site is one URL and every section is a hash route… that is the ceiling on organic search as the site is built, and lifting it means real routes, not more markup"* — and it was right, which is why it was acted on rather than left. `/how-it-works`, `/gallery`, `/reviews`, `/areas`, `/faq` and `/contact` are real addresses: Netlify rewrites each to index.html (`_redirects`), and the router reads the route from `location.pathname` when there is no hash. Each one sets its own `<title>`, description and canonical as it opens, so the seven are seven pages rather than seven copies of the homepage.
  ...
  - ⭐ **Each page is DOWNLOADED with its own head (2026-09-19).** Until then the six paths were served as the homepage (homepage `<title>`, description and `canonical` = `/`), and only became themselves once JavaScript ran. Google reads the downloaded canonical before rendering and says not to change it with JavaScript, so six pages were telling it "I am the homepage" and then "I am /faq". Link previews (iMessage, Facebook, Bing) run no JavaScript at all and showed the homepage for every one. `netlify/edge-functions/route-meta.js` now rewrites the title, description, canonical and share-card tags in the HTML Netlify sends, and adds a `BreadcrumbList`. It has `onError: 'bypass'`: if it throws, index.html goes out unchanged, which is how the site behaved before it existed. ⚠ **Its table is `ROUTE_META` written a second time.** Section 8 of `search-visibility.test.js` compares the two, and runs the function over the real index.html to prove every tag is still found. Change a title in both files. ⚠ **`HOME_TITLE` and `HOME_DESC` are literals now**, not read from the head. On `/faq` the downloaded head IS the FAQ's, so reading it would have put the FAQ title on the homepage after one click. The test holds them equal to the head.
  - ⭐ **The old site's addresses 301 (2026-09-19).** Before this app the domain was a WordPress site with a page per city (`/christmas-lights-installation-lehi-utah` and 21 more), service pages and a blog. Google still listed several of them, and every one answered 404, which throws away the ranking and the links they had built up. 47 of them now 301 in `_redirects`: city pages go to `/areas`, service and how-to pages to `/how-it-works`, blog questions to `/faq`, contact pages to `/contact`. The list came from the Wayback Machine's index of the domain plus a `site:` search. `/privacy`, `/terms` and `/accessibility` are left as 404 on purpose, because no such page exists yet. Section 9 checks that every 301 lands on a live page and none shadows one.
  - ⭐ **The gallery stopped costing every visitor 29.6 MB (2026-09-19).** Every page is this one document, and the ~190 gallery `<img>`s had no `loading="lazy"`, so the FAQ, the contact form and every quote link downloaded all of them in their original size. `cdnImg(url, width)` asks Cloudinary for a WebP or AVIF copy no wider than the slot (`f_auto,q_auto,c_limit,w_N`, and non-Cloudinary URLs are left alone), and lazy images inside a hidden page are never fetched. /faq went from 29.6 MB of images to 18 KB. The hero, How It Works and Areas pictures go through `cdnImg` too. Gallery alts read "Roofline Christmas lights installed by Highlighting Utah for <caption>": the caption is the customer's name, already printed under the photo. Section 10 gates it.
  - ⭐ **What people actually type, measured (2026-09-19).** Google Trends for November 2025, plus Google autocomplete:
    - **Wording:** "christmas light installation" beats every other wording. In the US, "christmas lights installation" drew about 60% as many searches. "installers", "hanging service", "professional …" and "company" were near zero. Utah alone is too small for Trends to show a single month, so Utah was read over five years.
    - **Utah, rising:** "residential christmas light installation" is the fastest-rising installation query in Utah (+600%), and this business is residential-only. The homepage `<title>` now leads with it.
    - **Cost:** "…cost", "how much…", "…cost per foot" and "…prices" are among the most searched installation queries. The site never said how a price is reached. How It Works now does ("priced per foot of roofline", all included). ⚠ It carries no dollar figure, because publishing the Per Foot Pricing rate is the owner's call.
    - **Permanent lights:** in Utah, "permanent christmas lights" is searched about 10× as often as "christmas light installation". The business sells seasonal only, so How It Works says plainly what seasonal means rather than pretending otherwise.
    - **Near me:** "who hangs christmas lights near me" and "people who hang christmas lights for you" are common autocompletes, but "near me" is won by the Google Business Profile, not the page.
    - **No history:** there is no Search Console property under highlightingutah@gmail.com, so there is no record of what people actually searched to reach this site.
  - **The head now says "residential", like the page.** The description and JSON-LD used to say "homes and businesses" and list commercial installs, while the hero says *Residential Only*. The JSON-LD also carries the four services (`hasOfferCatalog`) and a `WebSite` entity, which is where Google reads a site name from.

⚠ **The reviews on the site are placeholder content.** (The gallery was too until the Website tab filled it: on 2026-09-19 it showed 187 real photographs, most captioned with the customer's name.) `GALLERY` in index.html is nine hand-drawn SVG gradients of a generic house, and `REVIEWS` is six invented testimonials. On 2026-09-19 the live /reviews page was still showing those six, because the `reviews` collection is empty ("Kaitlyn R.", "Trevor M." and four more). Firestore overrides both when the Website tab has real rows. For a business whose work is entirely visual this is the biggest remaining gap in what search and a visitor actually see — real photographs of real rooflines, with captions, would do more for both than any markup. ⛔ **And the invented testimonials must never be marked up as reviews** — that is the same refusal as the 4.9 above, with the added problem that they are not real.

⭐ **The Privacy Policy is a real page at `/privacy` (2026-09-19)** — the eighth real path, in all four files section 7 of `search-visibility.test.js` compares. It says what the site actually does, including the Google tags and the hashed email/phone sent with a quote conversion; `ads-tracking.test.js` fails if the tag sends those and the page stops saying so. ⚠ **Terms & Conditions was REMOVED from the footer, not written.** Both links were `href="#"`; terms are the business's own promises about cancellations, payment and damage, and are the owner's to write. Put the link back with a page when they exist.
...
⚠ **No rating is marked up on the site, on purpose.** The 4.9 belongs to the Google profile. Review markup must describe reviews this site collected; copying the number in is what earns a manual action. `search-visibility.test.js` fails if anybody adds it.
