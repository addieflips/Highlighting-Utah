...
- **Copy their phone numbers, straight off the email filters** (Automation Emails → Preview & Send recipient list, 2026-09-24, [[RS-68]]). Addie: *"filter out pending numbers on automation email then RSVP then text the RSVP."* Set **RSVP Status → Not answered yet**, send the RSVP, then press **Copy their phone numbers** beside Select everyone matching and paste the column into the bulk texter. It copies **whoever the filters match** (not the ticks, not the search box), digits only, one per line — including people with **no email**, whom the email just skipped, and it says how many of those there were. STOP replies, the do-not-send list and anybody with no number are left out and each is counted on the line underneath. Numbers only: the per-person RSVP links are on the Text the RSVP card's link sheet.
  - **A batch at a time** ([[RS-69]], same day). Both copy buttons — this one and Text the RSVP's *Copy every phone number* — now open a panel: **How many at a time** (default 50), **Separate with** commas (for a To box, the default) or one per line (for a spreadsheet), the current batch shown in a box, and **Previous batch / Copy batch N / Next batch**. Batch 1 is copied on the first press. If the browser refuses the copy the numbers are left selected in the box and the line says press Ctrl+C. The size and separator are remembered on that browser.
