...
| `outboundTexts` | One row per text the app sends by itself, through the paid number: who, what, whether the provider took it, and what the carrier said about delivery. **This is what makes "did we text this person?" answerable** — it was not, for anything but a quote. Written even when the send FAILS, because a ledger that records only successes answers the question wrong in the one case that matters |
| `inboundTexts` | One row per text a customer sends us, keyed on the forwarding email's own message id. It exists to stop a retried post raising the same Inbox note twice — the note in `messages` is what anybody actually reads, and a STOP writes `smsOptedOut` on the customer as well |
...
- **`sendNightlyInvoices`** — cron, 7 PM Mountain daily (`0 19 * * *`). No-ops unless the automation toggle in `settings/nightlyInvoiceAutomation` is on. Bills any completed-but-uninvoiced house and logs to `nightlyInvoiceLog`. A run that hits an error, or that bills somebody with no email on file, also posts a **Nightly Billing Needs You** note to the System folder of the Inbox; a clean run stays quiet, because a note after every run trains the eye to clear the Inbox unread.
...
- ⭐ **THE APP CAN SEND TEXTS, FROM A SECOND NUMBER THAT IS NOT THE GOOGLE VOICE ONE (2026-09-11).** `sendTextToCustomer` posts to Telnyx on a toll-free number rented for the purpose. **It is not, and cannot be made to be, the 801 number** — a number is hosted by one provider, Google does not text-enable Voice numbers, and using the 801 would mean porting it out and losing the office's Google Voice inbox with it. Two numbers is the deliberate price of keeping that inbox free.
  - ⚠ **The opt-out is enforced on the send path, not only on the screens.** Every button in admin.html already refuses an opted-out customer; that is not enough, because a loop or a scheduled run has to remember separately and the one that forgets texts somebody who asked us to stop — from a *registered* number, which is the complaint that ends a campaign.
  - ⚠ **Toll-free, not local, on purpose.** Since Feb 2025 US carriers BLOCK unregistered A2P texts outright. A toll-free number is verified rather than 10DLC-registered, which avoids the monthly campaign fee on a line that sends rarely.
  - `telnyxWebhook` takes both directions back: `message.received` goes into the same `recordInboundText` the Google Voice script uses, and `message.finalized` writes the carrier's delivery verdict onto the `outboundTexts` row.
  - ⚠ **The webhook signature is checked (Ed25519, over `<timestamp>|<raw body>`, five-minute window).** It has to be: the webhook reaches the code that writes `smsOptedOut`, so an open one is a way for anybody to opt out the entire book with a forged STOP — and nothing would look wrong until a season of messages had gone nowhere.
  - ⛔ **NOTHING IN THE UI SENDS THROUGH IT YET.** The capability, the ledger and the webhook are built and tested; no screen has been switched over, because moving a customer-facing message from the number they know to a toll-free one is a decision about how the business looks, not a refactor.
- ⭐ **Inbound texts DO reach the app, and that is the half of Google Voice that can be automated.** Google Voice → Settings → Messages → *"Forward messages to email"* is a supported setting, not a workaround. `tools/google-voice-inbound.gs` is an Apps Script on that mailbox: every 5 minutes it reads the forwarded emails and posts them to the **`inboundText`** Cloud Function, which raises a System note in the Inbox and honours a STOP.
  - ⚠ **The parsing lives in the Apps Script on purpose.** The shape of a forwarded Google Voice email is Google's to change without telling anybody, so the brittle part sits where it can be fixed in a web editor in two minutes, and the Cloud Function is handed a number and a body it can trust — which is what makes it gradable without an inbox at all (Suite 328, 7 sabotages red-checked).
  - ⚠ **It fails towards telling somebody.** A message the script cannot parse is not dropped and not marked done: it is posted with the raw body under a placeholder number, flagged, and left for the next run as well. A silently skipped message could be a STOP. The function de-duplicates on the email's own message id, so the retry costs nothing.
  - ⚠ **Run `dryRun` in the Apps Script before arming the trigger.** It prints what it would have posted for the newest few emails and writes nothing. The email format has not been verified against a real forwarded message in this repo — that is what `dryRun` is for.
- ⚠ **Nothing on the server can send a text, and nothing tries to any more (2026-09-11).** Both owner alerts — the nightly billing summary and the unmatched-payment warning — used to call `twilioSendRaw`, on an account Highlighting Utah has never had. It returned `{ok:false}` and never threw, so both had failed silently since the day they were written, and the Automation screen described the nightly one as *"sent by text (not email), so it reaches you even when email is the problem"*. Both are System notes in the Inbox now, which is what the unmatched-payment path had already chosen on its own reasoning: *a text is gone the moment you look away; a note keeps until somebody deals with it*. **Anything that has to reach a phone is sent by a person from Google Voice.**
...
- **No button anywhere in this app sends a text, and five of them say so.** Google Voice has no send API, and driving a signed-in session breaks Google's own Acceptable Use Policy — so every texting screen goes through one function, `handToGoogleVoice(phone, msg)`: it copies the message, opens that customer's Google Voice conversation, and a person presses send.
  - The five: the quote text, the quote photo copy, the RSVP text, the bulk text, and the route sheet. They had drifted apart — one opened Google Voice, three said *"paste into your texting app"*, and the route sheet used an `sms:` link. Unified 2026-09-11, graded by **Suite 327** (6 sabotages red-checked).
  - ⚠ **The route sheet was the one that was actually wrong.** An `sms:` link opens whatever messages app the *device* has, so a crew member tapping a customer's name texted them **from a personal number** — one the customer cannot usefully reply to and nobody in the office can see.
  - Only the quote text records the send, on the office pressing *I sent it*. The other four write nothing, so "did we text this person?" is unanswerable for them.
  - The February invoice chase is deliberately different: its copy is a *worklist* (name, phone, amount, tab-separated for a spreadsheet), not one message, so its button opens the Google Voice **inbox** rather than a thread.
  - It used to claim it could. The button called a `sendSms` cloud function wired to Twilio, on an account Highlighting Utah has never had, so every press failed with *"Authentication Error - invalid username"* — after the quote had been priced and the office had been told it was sending. Fixed 2026-09-11.
  - `sendSms` and `twilioSendRaw` were deleted from `functions/index.js` on 2026-09-11, along with the three `TWILIO_*` secret declarations. ⚠ **The `sendSms` Cloud Function may still be deployed** — deleting the export removes it from the source, not from Firebase; it goes on the next `firebase deploy --only functions`, which will ask to confirm the deletion.
  - ⚠ **The "Text a nightly billing summary to this number" field is gone too**, with its Health Check row. The row told the office a blank number meant *"a run that stops firing is invisible"* — sending somebody to configure the one signal that could not work, and competing with the 36-hour stale-run check that does work and needs nothing set up.
- **The two opt-out flags, and what each one does NOT stop.** Both live on `jobAddresses` and both are deliberately narrow.
  - `smsOptedOut` (+ `smsOptedOutReason`, + `smsOptedOutAt`) — set two ways. By hand, via the **Don't text this customer** tick in Edit Customer; and automatically by **`inboundText`** when a customer replies STOP. Twilio used to be what caught a STOP (error 21610 on the next send); that went with Twilio, leaving nothing watching for a week, and `inboundText` is what watches now.
    - ⚠ **It matches the WHOLE message against the carrier keywords, never a substring.** *"stop by tomorrow and see the lights"* is a customer making a plan and *"please stop texting me about this"* is a complaint to be read — opting either of them out silently would be close to irreversible, because nobody ever tells you they stopped hearing from you. Suite 328 holds that line with five worked examples.
    - ⚠ **A friendly reply does not lift an opt-out, and neither does "yes".** Only START / UNSTOP / SUBSCRIBE. Somebody who asked not to be texted may still answer a question, and reading that as permission overturns a withdrawn consent using the customer's own politeness.
    - ⚠ **A STOP from a number nobody matches ticks nothing**, and the Inbox note says so in those words — otherwise the office reads "they opted out", assumes it was handled, and the next send texts them again. It hides the "Send as text instead" button on the quote card. It does not affect email.
