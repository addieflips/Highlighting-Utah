...
- **The two opt-out flags, and what each one does NOT stop.** Both live on `jobAddresses` and both are deliberately narrow.
  - `smsOptedOut` (+ `smsOptedOutReason`, + `smsOptedOutAt`) — set **only by hand**, via the **Don't text this customer** tick in Edit Customer, which is also what stamps `smsOptedOutAt`. It used to be set automatically: Twilio refused an opted-out number with error 21610 and the quote screen recorded that. Nothing detects a STOP any more (2026-09-11) — we text from Google Voice, where a STOP reply is an ordinary message somebody has to read and act on, and Google Voice will happily send to a number that has sent one. **If a STOP is not ticked in here, nothing else will catch it.** It hides the "Send as text instead" button on the quote card. It does not affect email.
