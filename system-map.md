...
13. **Colour change** — a house that already has lights up and asks for different ones is neither a build nor a recycle, and since 2026-09-10 it has its own warehouse queue (**Warehouse → Color Change**, [[WH-30]]). `needsColorChange` puts them on it, `colorChangeColors` is a COPY of the colours taken at the moment the change was recognised, and **Mark Done** clears the flag while keeping the colours — they are the only record of what was actually made up.
    ...
    ⭐ **The $30 is charged automatically, not asked about** ([[MON-70]], superseding [[MON-03]]). Until 2026-09-10 the Edit Customer save opened a popup with three answers — charge, waive, cancel — because waiving had to be possible at the point of saving. [[MON-54]]'s × on every fee line changed that: there is a way back now, so the save charges and the × undoes it. **Who is charged has not moved** — `applyLightChange` still refuses a first-time colour, a cleared one and anything inside the 48-hour free window — and **all four doors charge** ([[MON-71]]): the save and both portal writes through `applyLightChange`, the Color Change button and the All Customers panel through `chargeColorChangeFee`. ⚠ That reverses the narrower scope picked an hour earlier, and the objection it protected stands on the record — the button exists for a bundle WE lost or built wrong, so it can bill a customer for our own mistake; the × is how that comes back off. ⚠ The 48-hour free window survives: the customer already paid for the change that opened it, and charging inside it bills one person twice for one decision. ⚠ A first-time colour still pays nothing through the save, because it never reaches the list — not in colour change, no fee — but it DOES pay through the button, which is the answer for a house whose old colours were never recorded. ⚠ `waiveLedgerLine` now writes a history line for any waived fee: it DELETES the `changeFeeNotes` entry, so without that a waiver would leave no trace at all, which is the asymmetry that was closed once already.
