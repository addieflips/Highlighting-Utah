...
3. **Customer approves** — the customer opens that link (no login needed) and approves or declines. This calls the `quoteRespond` Cloud Function, which is how an unauthenticated visitor is allowed to touch the `quotes` collection at all.
   ...
   - ⭐ **A call that never got an ANSWER is asked again** (2026-09-11). The twelve urgent Member Errors raised from RSVP links on 8–10 September all reported `deadline-exceeded` or `internal`. Neither is the server refusing anything — they are the only two codes the callable **client** raises when the request never completed at all: its own 70-second deadline, and the fetch failing. `portalRsvp` runs at **minInstances 0** and its Cloud Run log shows most invocations starting a new instance, so the slowest call this site makes is the one a customer makes from an email, on a phone, once. `callPortalFn` made exactly one attempt, so each of those was an answer **lost** to a single cold start — and to that customer it looks like they already told us. It now retries `deadline-exceeded`, `internal`, `unavailable` and `aborted` three times with a short backoff, and **never** retries a server that has decided (`not-found`, `invalid-argument`, `permission-denied`), so the out-of-date-link wording above still arrives at once. Retrying is safe because every portal callable **sets** a field rather than counting or creating one — do not add a caller that ADDS anything without giving it its own key. Proved by SCENARIO 8 of `portal-repro.test.js`.
