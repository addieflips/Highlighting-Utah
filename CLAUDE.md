...
Baseline on 2026-08-14, AFTER the nine money/security fixes: 289 passed, 0 failed, 3 notes, 1 known GAP (two customers sharing a phone share one invoice key). Plus 9 on gate A, 8 on the selector contract, and 12 on money parity. The earlier line here read "232 passed … 2 known GAPs (colorComboKey; …)" — the colorComboKey gap has since closed and the suite has grown; trust a fresh `npm test` over any number written down here.
⚠ Suite 10 of run-all.js is ASYNC — it EXECUTES syncPayerInvoice against a fake Firestore rather than reading it as text, because a regex cannot catch an undefined variable and a text-only check let the `forTotal` crash ship. Anything async must be pushed onto `pendingAsync`; the summary awaits that list before printing. A check that scores after the summary can never fail the build.
...
.github/workflows/tests.yml runs on every push to main and on every pull request. Two jobs:
  ...
  browser-tests — Playwright, with chromium installed via `npx playwright install --with-deps chromium`. Marked continue-on-error: true because several specs deliberately target known Member Portal bugs and are RED ON PURPOSE. Failure screenshots and traces upload as an artifact, kept 7 days.
  ⚠ CORRECTED 2026-08-14: this said "five specs" and named t9/t11/t14/t15/t17. It is SEVEN on main today — those five plus "a wrong last name is refused, and says so" and "a customer who cancelled sees the turned-off message, not their account". Both drifted red at some point after that note was written and are unrelated to the money/security batch (verified by running the suite at that branch point and after: 7 failed, 1 passed, identically). Before blaming your own change for a browser failure, get the baseline the same way. `npm run test:browser` needs `npm install` first — @playwright/test is a devDependency and is NOT installed by the fast suite's setup.
  ⚠ DELETE the continue-on-error line the moment those specs go green. A job that is permanently allowed to fail is a job nobody reads, and at that point it is worse than not having it.
