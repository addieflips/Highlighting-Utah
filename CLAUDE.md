...
admin.html	Office dashboard (~1.5MB; two inline scripts — a plain one and a <script type="module">, which is the app itself)
js/test-seed.js	The Project To-Do checklist's 202 manual tests (TEST_SEED), moved out of admin.html on 2026-08-14. Loaded by a dynamic import() inside runProjectTestSync, so it is fetched only when the checklist syncs (right after login), never blocks the dashboard rendering, and is cached separately from the page — an admin.html change no longer re-sends 217KB of test list. ⚠ A failed fetch is caught and reported on screen; do not remove that catch, because the only caller swallows errors and the checklist would silently stop syncing.
