...
Two new gaps found during the 2026-08-08 audit, left as gap() checks in run-all.js rather than blind-fixed (need an owner decision):
Invoice Bulk Update (admin.html, ibImportBtn) always zeroes removal and deposit for every row, even when an invoice already exists with a removal charge or a recorded payment — silently erasing both if the tool is ever re-run against an existing invoice.
Switching a customer's bill-to never moves an outstanding light-change fee onto the new payer's invoice — an unpaid $30 fee can end up billed to nobody.
...
The two new gaps above — need an owner decision on intended behavior before anyone fixes them.
...
Needs a human: first-time firebase login / git credentials; anything requiring the master customer sheet or a business judgement call (which duplicate keeps a number, whether to re-lock the quotes rule, how the two new gaps in §4 should behave); a real PayPal test checkout; approving irreversible data changes; confirming a Cloud Functions deploy that got stuck (see §1).
