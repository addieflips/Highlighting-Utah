...
.github/workflows/tests.yml runs on every push to main and on every pull request. Two jobs:
  ...
  browser-tests — Playwright, with chromium installed via `npx playwright install --with-deps chromium`. Marked continue-on-error: true because some specs deliberately target known Member Portal bugs and are RED ON PURPOSE. Failure screenshots and traces upload as an artifact, kept 7 days.
  ⚠ CORRECTED 2026-08-14: this said FIVE red specs (t9, t11, t14, t15, t17). It is TWO — t11 and t17. t15 went green earlier; t9 and t14 went green on 2026-08-14 once the harness was fixed, and neither was ever a product bug: every invoice fixture omitted name/phone/email though INVOICE_READ_FIELDS begins with exactly those three, and the stub returned invoice fields FLAT while production returns {found, record} and takes `key`. index.html reads res.record, so every field came back undefined. Worth knowing before chasing a red spec into the page: check the fixture matches production's shape FIRST (§9.13).
  ...
  ⚠ DELETE the continue-on-error line the moment those two specs go green. A job that is permanently allowed to fail is a job nobody reads, and at that point it is worse than not having it.
