...
| PROC-18 | What else should the connections map carry to be usable? | All four, in this order: **list what it does NOT watch** (so a green page is never read as "the app is fine"), **the remaining ten spines**, **the broken count on the Project nav item** so a break is visible from anywhere, and **when the map was last rebuilt** | 2026-08-26 | `NOT_WATCHED`, `paintChecklistBadge`, `builtFrom` | Standing |
