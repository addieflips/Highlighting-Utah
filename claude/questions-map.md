...
| HC-03 | The Health Check panel itself has no way to accept a row — what should it offer? | Three verdicts per row. "I can't approve anything or make an exception for certain people so all health check just sits there forever." **Fixed it** (she changed the record), **Not now** (real, not today), **Fine as it is** (the exception). ⚠ The mechanism is HC-01's, on the panel rather than on a notice: keyed on check + row + fingerprint. HC-01 stays *Decided — not built* — that row is about approving from a **message**, which this does not build | 2026-08-26 | `hcRowState`, `hcDecide`; Suite 7 | Standing |
| HC-04 | How long does **Fine as it is** last? | Forever — **unless the details change.** Each decision stores a fingerprint of the row's `detail`, and a row whose facts have moved comes back: a drifted amount becoming a *different* amount, a group of two houses becoming three. ⚠ Without it one click blinds that row permanently, which is the same class of failure as a badge nobody ever saw | 2026-08-26 | `hcFingerprint`, `hcRowState`; Suite 7 | Standing |
| HC-05 | Is a decision private to whoever made it, or shared? | Shared across the office, stamped with who made it and when. Four people use this dashboard; one person clearing a row clears it for everyone | 2026-08-26 | `healthCheckDecisions`; Suite 7b | Standing |
| HC-06 | Can **Nightly billing has stopped** be set aside? | Never. Put off for at most **7 days**, and that is all. An outage is not an exception — the row means installed houses are silently never invoiced. ⚠ Refused on READ as well as on write, so a document written by hand or by an older build cannot blind it. Everything else, *New-message alerts are switched off* included, can be set aside | 2026-08-26 | `HC_NEVER_SET_ASIDE`, `HC_SNOOZE_CAP_DAYS`; Suite 7 | Standing |
