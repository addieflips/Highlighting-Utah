...
| HC-03 | ~~Health Check is not being read — rebuild the panel?~~ | ~~**No — "don't do anything with design."**~~ ⚠ IT WAS A "NOT NOW", AND THE NOT-NOW ENDED on 2026-08-27, when she handed over a written redesign plan and asked for it to be built — HC-04. ⭐ HER REASON HERE IS WHY HC-04 IS RIGHT, and is the whole value of keeping this row: "I don't check health check very often cause the design is weird and I can't mark anything as completed or outside of policy." Both halves were verified in code at the time — there was no dismiss or acknowledge anywhere in `hcRunChecks`/`hcRender`, and the badge counted ROWS, so lists she had already ruled on (`sharedPhone` alone is 14 genuine households out of 17) held it permanently above zero, and a number that can never reach nought is one nobody reads. Both are now fixed, and the badge counts only rows awaiting a decision. ⚠ THE CONSEQUENCE THIS ROW RECORDED IS THEREFORE CLOSED: Health Check was the delivery route four findings were queued behind (Q-019's `phoneNotDigits`, `strandedPayment`, `townIsStreet`, and the colour wipe), which is why a finding that mattered had to be reported to her directly instead of filed in the panel. Filing one there is viable again. ⚠ NOT a reversal of HC-01/HC-02, which are the same marking mechanism asked for from the NOTICE end and are still Decided — not built | 2026-08-26 | `hcRender` (badge = totalRows); no dismiss state existed | Superseded → HC-04 |

| HC-04 | The Health Check panel itself has no way to accept a row — what should it offer? | **Supersedes HC-03**, which was a "not now" on exactly this rebuild the day before — she then handed over a written redesign plan and asked for it to be built. Three verdicts per row. "I can't approve anything or make an exception for certain people so all health check just sits there forever." **Fixed it** (she changed the record), **Not now** (real, not today), **Fine as it is** (the exception). ⚠ The mechanism is HC-01's, on the panel rather than on a notice: keyed on check + row + fingerprint. HC-01 stays *Decided — not built* — that row is about approving from a **message**, which this does not build | 2026-08-26 | `hcRowState`, `hcDecide`; Suite 7 | Standing |
| HC-05 | How long does **Fine as it is** last? | Forever — **unless the details change.** Each decision stores a fingerprint of the row's `detail`, and a row whose facts have moved comes back: a drifted amount becoming a *different* amount, a group of two houses becoming three. ⚠ Without it one click blinds that row permanently, which is the same class of failure as a badge nobody ever saw | 2026-08-26 | `hcFingerprint`, `hcRowState`; Suite 7 | Standing |
| HC-06 | Is a decision private to whoever made it, or shared? | Shared across the office, stamped with who made it and when. Four people use this dashboard; one person clearing a row clears it for everyone | 2026-08-26 | `healthCheckDecisions`; Suite 7b | Standing |
| HC-07 | Can **Nightly billing has stopped** be set aside? | Never. Put off for at most **7 days**, and that is all. An outage is not an exception — the row means installed houses are silently never invoiced. ⚠ Refused on READ as well as on write, so a document written by hand or by an older build cannot blind it. Everything else, *New-message alerts are switched off* included, can be set aside | 2026-08-26 | `HC_NEVER_SET_ASIDE`, `HC_SNOOZE_CAP_DAYS`; Suite 7 | Standing |
